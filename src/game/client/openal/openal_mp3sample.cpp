...
static FORCEINLINE const char *MadErrorString( const struct mad_stream *Stream )
{
	switch ( Stream->error )
	{
...
static FORCEINLINE unsigned long prng( unsigned long state )
{
	return ( state * 0x0019660dL + 0x3c6ef35fL ) & 0xffffffffL;
}

static mad_fixed_t audio_linear_dither( unsigned int bits, mad_fixed_t sample,
								 ...
								 struct audio_stats *stats )
{
	...
	/* bias */
	mad_fixed_t output = sample + ( 1L << ( MAD_F_FRACBITS + 1 - bits - 1 ) );

	unsigned int scalebits = MAD_F_FRACBITS + 1 - bits;
	mad_fixed_t mask = ( 1L << scalebits ) - 1;

	/* dither */
	mad_fixed_t random = prng( dither->random );
 ...
 */
static unsigned int audio_pcm_s16le( unsigned char *data, unsigned int nsamples,
							  ...
							  struct audio_stats *stats )
{
	...
	{  /* stereo */
		while ( len-- )
		{
			const mad_fixed_t &sample0 = audio_linear_dither( 16, *left++, &left_dither, stats );
			const mad_fixed_t &sample1 = audio_linear_dither( 16, *right++, &right_dither, stats );
 ...
 */
static unsigned int audio_pcm_s16be( unsigned char *data, unsigned int nsamples,
							  mad_fixed_t const *left, mad_fixed_t const *right,
...
void COpenALMp3Sample::Open( const char* filename )
{
	...
	// Reset input data
	Input.data = static_cast<unsigned char *>( malloc( INPUT_BUFFER_SIZE ) );
	Input.length = 0;
	...
	while ( size < OPENAL_BUFFER_SIZE )
	{
		...
		if ( Stream.buffer == NULL || Stream.error == MAD_ERROR_BUFLEN )
		{
			...
			if ( Stream.next_frame != NULL )
			{
				...
				V_memmove( Input.data, Stream.next_frame, Input.length = &Input.data[Input.length] - Stream.next_frame );
			}

			// Execute the read operation, keeping the buffer size constant
			int len = filesystem->Read( Input.data + Input.length, INPUT_BUFFER_SIZE - Input.length, mp3File );
			...
			if ( filesystem->EndOfFile( mp3File ) )
			{
				...
				V_memset( GuardPtr, 0, MAD_BUFFER_GUARD );
		...
		int nsamples = Synth.pcm.length;    // Number of samples
...
public:
	...
	virtual IOpenALSample* Get() { return new COpenALMp3Sample; }
};

static COpenALMp3LoaderExt g_mp3Loader;
IOpenALLoaderExt *mp3Loader = &g_mp3Loader;