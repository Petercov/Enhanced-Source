 ...
 ***/
void IOpenALSample::SetPositional( bool positional )
{
...
void IOpenALSample::SetPosition( const Vector &position )
{
	V_memcpy( m_fPosition, position.Base(), 3 * sizeof( float ) );
...
{
	V_memcpy( m_fVelocity, velocity.Base(), 3 * sizeof( float ) );
 ...
 ***/
void IOpenALSample::SetEntity( CBaseEntity *ent )
{
	m_pLinkedEntity = ent;
}