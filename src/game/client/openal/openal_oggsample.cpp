...
static size_t vorbis_read_func( void *ptr, size_t size, size_t nmemb, void *datasource )
{
	return filesystem->Read( ptr, size*nmemb, static_cast<FileHandle_t>( datasource ) ) / size;
}

static int vorbis_close_func( void *datasource )
{
	filesystem->Close( static_cast<FileHandle_t>( datasource ) );
...
static int vorbis_seek_func( void *datasource, ogg_int64_t offset, int whence )
{
	...
	FileHandle_t fileDesc = static_cast<FileHandle_t>( datasource );
	...
	ogg_int64_t newPos = filesystem->Tell( fileDesc );
...
static long vorbis_tell_func( void *datasource )
{
	return filesystem->Tell( static_cast<FileHandle_t>( datasource ) );
}
...
void COpenALOggSample::UpdateMetadata()
{
	...
	for ( int i = 0; i < vorbisComment->comments; ++i )
	{
		...
		char *thisKey = ( char* )malloc( count + 1 );
		Q_strncpy( thisKey, vorbisComment->user_comments[i], count + 1 );

		char *thisValue = ( char* )malloc( vorbisComment->comment_lengths[i] - ( count + 1 ) );
...
public:
	...
	virtual IOpenALSample* Get() { return new COpenALOggSample; }
};

static COpenALOggLoaderExt g_oggLoader;
IOpenALLoaderExt *oggLoader = &g_oggLoader;