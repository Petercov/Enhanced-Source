...
typedef struct openal_groupdata_s
{
	CUtlString name; // The full-text name of this group
	CUtlVectorMT<CUtlVector<IOpenALSample*>> samples;

	openal_groupdata_s( const char* name )
	{
		this->name = name;
	}
} openal_groupdata_t;
...
public:
	...
	IOpenALSample* GetSample( const char* filename );
	 ...
	 ***/
	openal_groupdata_t* FindGroup( const char* name );
	void RemoveSampleGroup( const char* name );
	void AddSampleToGroup( const char* groupName, IOpenALSample *sample );
	void RemoveSampleFromGroup( const char* groupName, IOpenALSample *sample );
