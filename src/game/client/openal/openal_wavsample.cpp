...
static FORCEINLINE unsigned short readByte16( const unsigned char buffer[2] )
{
	//return (buffer[0] << 8) + buffer[1];
	return ( buffer[1] << 8 ) + buffer[0];
}

static FORCEINLINE unsigned long readByte32( const unsigned char buffer[4] )
{
...
bool COpenALWavSample::CheckStream( ALuint buffer )
{
	...
	while ( size < bytesToRead )
	{
		...
		else
		{
			...
			{
				Warning( "Wav: An error occured while attempting to read %s.", m_pszFileName.String() );
				return false;
			}
			else
			{
...
static COpenALWavLoaderExt g_wavLoader;
IOpenALLoaderExt *wavLoader = &g_wavLoader;
