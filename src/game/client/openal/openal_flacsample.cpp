...
void COpenALFLACSample::Open( const char* filename )
{
	...
	FLAC__StreamDecoderInitStatus status = Stream::init();
...
FLAC__StreamDecoderWriteStatus COpenALFLACSample::write_callback( const FLAC__Frame *frame, const FLAC__int32 * const buffer[] )
{
	...
	if ( frame->header.channels == 2 )
	{
		...
		for ( unsigned int i = 0; i < frame->header.blocksize; i++ )
		{
			...
			{
				int32 sample0 = buffer[0][i];
				int32 sample1 = buffer[1][i];
...
public:
	...
	virtual IOpenALSample* Get() { return new COpenALFLACSample; }
};

static COpenALFLACLoaderExt g_flacLoader;
IOpenALLoaderExt *flacLoader = &g_flacLoader;